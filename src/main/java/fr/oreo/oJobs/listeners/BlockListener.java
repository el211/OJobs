...
public class BlockListener implements Listener {
    ...
    public BlockListener(OJobs plugin) {
        this.plugin = plugin;
    }
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        var mat    = event.getBlock().getType().name();

        Optional<PlayerData> dataOpt = plugin.getPlayerDataManager().getPlayerData(uuid);
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        var uuid   = player.getUniqueId();
        var world  = player.getWorld().getName();
        var mat    = event.getBlockPlaced().getType().name();
        ...
        for (Job job : plugin.getJobManager().getJobs()) {
            ...
            String cooldownKey = job.getId() + ":block-place";
            if (data.isOnCooldown(cooldownKey)) continue;

            job.getBlockPlaceEntry(mat).ifPresent(entry -> {
                if (!meetsLevelReq(data, job, entry)) return;
                if (!entry.roll()) return;
                plugin.getPlayerDataManager().giveXp(player, job, entry.getXp());
                long cd = job.getCooldown("block-place");
                if (cd > 0) data.setCooldown(cooldownKey, cd);
            });
        }
    }
